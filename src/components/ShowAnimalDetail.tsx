...
export const ShowAnimalDetail = ({ currentAnimal }: IDetailedAnimalProps) => {
	...
	const handleClick = (animal: IAnimal) => {
		...
		const animalToChange = animals.find(
			(animalToChange) => animalToChange.id === Number(animal.id)
		);
		if (animalToChange) {
	...
	const feedTime = () => {
		...
		const feedCooldown = 3 * 60 * 60 * 1000; //3 hours in milliseconds

		if (timeDifference > feedCooldown) {
			return true;
		} else {
			setTimeout(() => {}, feedCooldown - timeDifference);
			return false;
		}
	};
	return (
